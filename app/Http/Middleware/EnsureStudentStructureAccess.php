...
class EnsureStudentStructureAccess
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user->hasSystemRole('school_manager')) {
            if ((int) $school->manager_user_id !== (int) $user->id) {
                abort(403, 'Only the assigned manager can access the student structure.');
            }
        ...
        if (!$user->canManageStudentStructure()) {
            abort(403, 'You do not have permission to manage the student structure.');
        }
