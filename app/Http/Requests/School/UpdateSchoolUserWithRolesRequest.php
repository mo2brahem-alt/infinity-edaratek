...
class UpdateSchoolUserWithRolesRequest extends FormRequest
{
    ...
    {
        /** @var User|int|string|null $boundUser */
        $boundUser = $this->route('user');
        $userId = $boundUser instanceof User ? $boundUser->id : (int) $boundUser;

        return [
            'name' => ['required', 'string', 'max:255', 'min:3'],
            'email' => ['required', 'string', 'email:filter', 'max:255', Rule::unique('users', 'email')->ignore($userId)],
            'mobile' => ['required', 'string', 'max:20', new SaudiMobile, Rule::unique('users', 'mobile')->ignore($userId)],
            'department_id' => [
                'required',
